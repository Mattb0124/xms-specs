...
- [~] Hardening (P1.8.1): rate limiting on the portal, webhook and public routes with `abuse.rate_limited` done 2026-09-07 (backend `4184faa`); CORS allowlist and helmet in place; CSP report endpoint (`POST /v1/csp-report`, `abuse.csp_violation`) and the dependency audit script done 2026-09-07 (backend `HEAD`); image-size through pptxgenjs has no patched release (exception recorded in WHAT-WAS-DONE); the ZAP baseline waits for the deployed hosts
