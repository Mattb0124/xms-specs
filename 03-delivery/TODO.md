...
- [x] Calendars and time zones driving SLA (P3.26.1): business calendar engine, holiday library, calendar documents with the account default and preview, SLA clocks bound to the calendar they started on, at-risk judged on working minutes, 2026-09-07 (backend); after-hours class on time entries (TB-13: derived from the account calendar and an optional local start, contract handling premium rate or comp time with the multiplier frozen on the row, `PATCH` contract handling, comp-time report) 2026-09-07 (backend `fd9ff20`, migration 0015); the web app's start-time field, class badge and contract handling are with a frontend agent; Calendars tab on the account record, calendar editor with the hours grid and preview panel, holiday libraries screen, 2026-09-07 (frontend `9318fe2`, `568fba0`); a repeated date in a posted library is now refused (backend `cc6e489`)
- [~] Rate cards, forecast, thresholds, adjustments, billing periods and export, finance connector, renewal alerts: rate cards as versions per effective date with the rate and amount frozen on the entry, forecast over business days from the account calendar, thresholds fired once per period with notification, outbox and audit, overage rule (block, flag, rate), rollover carry-over at period open, contract rules on create and patch, `GET /accounts/{id}/budget` and the drill-through, 2026-09-07 (backend `b5fffe3`, migration 0016; 6 scenarios); the Budget view, rate card and contract rules screens, the finance export file (TB-14), the finance connector and renewal alerts remain
