...
- [~] Hardening (P1.8.1): rate limiting on the portal, webhook and public routes with `abuse.rate_limited` done 2026-09-07 (backend `4184faa`); CORS allowlist and helmet in place; ZAP baseline, the CSP report endpoint and the dependency audit in the gate wait for the deployed hosts
